...
def test_stamp_raw_lineage_omits_source_uri_when_absent(spark: SparkSession) -> None:
    ...
    stamped = lineage.stamp_raw_lineage(raw, stamp)

    assert set(stamped.columns) == {"x", "batch_id", "delivery_id", "feed_id", "received_at"}
...
def test_stamp_functions_add_only_the_declared_columns_no_more(spark: SparkSession) -> None:
    ...
    raw_stamped = lineage.stamp_raw_lineage(df, stamp)
    fact_stamped = lineage.stamp_fact_lineage(df, stamp)

    added = {"batch_id", "delivery_id", "feed_id", "received_at"}
    assert set(raw_stamped.columns) == before | added
    assert set(fact_stamped.columns) == before | added
