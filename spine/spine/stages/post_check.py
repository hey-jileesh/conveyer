...
stage in this milestone.** `core/model.py`'s `ChecksModel` (K7) refuses ANY
`batch_check` entry at spec-PARSE time, unconditionally, until the 005 v1.x
member grammar lands — no validly-bound `PipelineSpecModel` can carry one,
so `business_checks.compile_business_checks`'s own `isinstance` filter
already excludes `BatchCheckModel` structurally (its own docstring). This
stage therefore implements no `batch_check` evaluation/verdict-rendering
code of its own (G-05(a)'s bind-defect coverage is the whole story here;
G-05(b-h) are 005 v1.x's named wait, 006.1 §14's B4 row).
