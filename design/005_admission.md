...
**Status:** v1.0 — reviewed and binding · **Parent:** *001 Batch Data Processing Architecture* (§2.2, §5) · **Plan:** 003 §3.2 · **Spine:** 004 §5–§6 (context and stage protocol — normative, unaltered here) · **Position:** stages 1–2 of the sequence; the boundary where a registered delivery becomes rows · **Pattern:** remember (raw) + validate (shape); tolerant reader, strict projection

> **Conventions.** Positions are recorded as decisions **D-1…D-11** in Y-statement form so 006–009 and 012 can cite both the choice and its cost; all are settled as of v1.0. v1 means *reviewed and binding*, not final — friction from 005.1 and from drafting 006–008 lands as v1.x revisions with decision-record updates (003 §3). Every section after the decision record opens with the problem it answers. This doc discharges 004 §16.3 (unreadable rows) and the 005 register of 004.1 §15.2 and the 004.1 errata notes §2; each obligation is cited where it lands. Detail that belongs to the LLD (005.1) or another cluster doc is named and deferred, not sketched here.
...
1. **Authoring file format and home** for the raw contract + read spec (one file or two, schema format) → 009, needing Track A's provisional answer (003 §4). **Interim stance, consciously accepted at v1.0:** 005.1 extends the runner-side `PipelineSpecModel` with typed `RawContract`/`ReadSpec` models replacing I-P1/I-P2 — the 004.1 §6.2 pattern: runner-consumed model now, authored surface frozen at 009. Two guards against the provisional shape freezing by inertia: `check_version`/`read_spec_version` are defined over the canonical *parsed* form (content hash), never the authored file, so a 009 re-homing cannot silently change version identity; and 009 inherits a recorded obligation to **decide** the authored surface afresh, not to ratify 005.1's parse models by default.
