...
- **M-1 — Facts, not a pointer.** Marker rows are append-only facts under the same IAM regime as the fact tables; "latest completed" is a derived read, never a stored/updated value. If a marker row can be UPDATEd, the table has become a mutable pointer — value re-complected with time — and the silent-staleness hole returns through the back door.
  ...
  > **Annotation [DS2-1]** (2026-08-30, bead `conveyer-hpp.13.24` — erratum note per house convention; the original text above stands unaltered): "under the same IAM regime as the fact tables" is **scoped by 007.1 §6.1/§6.3 [DS2-1]**. What that regime enforces is *physical* no-delete (no spine role holds any S3 delete) and *cross-principal* isolation (004.1 [S-5]'s per-table grants); it does **not** construct own-writer append-only — the own-lane writer role irreducibly holds `glue:UpdateTable` + `s3:PutObject`, which suffice to commit a logical rewrite snapshot. Own-writer logical rewrite is therefore a **named residual** at I-21's authored-code/CI-linted boundary, with a registered **008 snapshot-operation audit** as its detection layer (007.1 §16 [DS2-1]: fact + marker tables alarm on any snapshot operation ≠ append; state tables on operations outside {merge, rebuild-window overwrite}) — the same premise correction the [DS-2] table-class ruling already made (007.1 F-10: job-role-mutable ⇒ provenance, never protection), one rule about Glue-grain IAM, not two incidents. M-1's substance stands unchanged: rows are facts, latest-completed is a derived read, and an UPDATE-capable marker remains disqualified.
