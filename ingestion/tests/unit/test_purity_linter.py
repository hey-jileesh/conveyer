"""Unit tests for `tools/purity_linter.py` (LLD §12.2).

`tools/` is not itself a package (it lives outside `ingestion/`, deliberately
not part of the walked tree — see the module docstring in purity_linter.py),
so it is imported here via an explicit `sys.path` entry rather than a
...
def test_must_fail_fixture_reports_violation(
    ...
    violations = purity_linter.lint_source(_fixture_source(fixture_name), rel_path)
...
def test_validator_raise_exemption_covers_bare_and_attribute_decorator_forms() -> None:
    ...
    violations = purity_linter.lint_source(source, "ingestion/core/x.py")
...
def test_real_model_and_completeness_modules_are_purity_clean() -> None:
    ...
    for rel in ("ingestion/core/model.py", "ingestion/core/completeness.py"):
        ...
        violations = purity_linter.lint_file(path, root)
...
def test_uuid5_is_not_banned() -> None:
    ...
    violations = purity_linter.lint_source(source, "ingestion/core/ids.py")
    assert violations == ()
...
def test_effects_and_entrypoints_are_exempt_from_purity_but_not_idiom() -> None:
    ...
    assert purity_linter.lint_source(source, "ingestion/effects/s3.py") == ()

    # a banned-everywhere idiom import still applies to effects/
    mock_source = "from unittest import mock\n"
    violations = purity_linter.lint_source(mock_source, "ingestion/effects/s3.py")
...
def test_sources_tree_is_full_purity_scope() -> None:
    ...
    violations = purity_linter.lint_source(source, "sources/carrier-x/pull.py")
...
def test_discover_files_only_walks_ingestion_and_sources(tmp_path: Path) -> None:
    ...
    found = purity_linter._discover_files(tmp_path)
...
def test_lint_file_reports_rel_path_and_finds_real_violation(tmp_path: Path) -> None:
    ...
    violations = purity_linter.lint_file(bad_file, tmp_path)
...
def test_main_returns_zero_and_prints_nothing_when_tree_is_clean(
    ...
    monkeypatch.setattr(purity_linter, "__file__", str(fake_tools_dir / "purity_linter.py"))

    exit_code = purity_linter.main()
...
def test_real_repo_scaffold_walk_matches_current_source_tree() -> None:
    ...
    root = Path(__file__).resolve().parents[2]
    files = purity_linter._discover_files(root)
    assert files, "expected to discover at least one real source file"

    all_violations = [v for f in files for v in purity_linter.lint_file(f, root)]
